...
covering_routes = Dict{Tuple{Int, Int}, Vector{Int}}()
route_costs = [Vector{Float64}() for _ in facilities] 
for j in facilities
    for i in customers
        covering_routes[(j,i)] = Vector{Int}()
    end   
    @show route_costs
    for (q,r) in enumerate(routes_per_facility[j])
        for i in r.path
            if i in customers
                append!(covering_routes[(j,i)], q)
            end
        end
        cost = 0.0
        for i in 2:r.length
            cost += arc_costs[r.path[i-1], r.path[i]]
        end
        append!(route_costs[j], cost)
    end
...
#sp variables
@variable(model, 0 <= λ[f in axis, j in facilities, q in 1:length(routes_per_facility[j])] <= 1) # λj,q = 1 -> route (j,q) is opened

@constraint(model, open[fake in axis, j in facilities, q in 1:length(routes_per_facility[j])], 
    y[j] >= λ[fake, j, q])

@constraint(model, cover[fake in axis, i in customers], 
    sum(λ[fake, j, q] for j in facilities, q in covering_routes[(j,i)]) >= 1)
...
@objective(model, Min,
    sum(facilities_fixed_costs[j] * y[j] for j in facilities) + 
    sum(route_costs[j][q] * λ[fake, j, q] for j in facilities, q in 1:length(routes_per_facility[j])))               
