...
"""
@enum(
    SolutionStatus, FEASIBLE_SOL, INFEASIBLE_SOL, EMPTY_SOL, UNKNOWN_SOLUTION_STATUS,
...
function MoiResult(model::M) where {M<:AbstractModel}
    ...
    return MoiResult{M,S}(
        UNKNOWN_TERMINATION_STATUS, UNKNOWN_SOLUTION_STATUS, PrimalBound(model),
        DualBound(model),
...
function MoiResult(
    model::M, ts::TerminationStatus, ss::SolutionStatus; pb = nothing,
...
function determine_statuses(res::MoiResult, fully_explored::Bool)
    ...
    if !found_sols # Implies that gap is not zero
        setsolutionstatus!(res, EMPTY_SOL)
...
function retrieve_result(form::Formulation, optimizer::MoiOptimizer)
    ...
    if terminationstatus != MOI.INFEASIBLE &&
        ...
        setsolutionstatus!(result, EMPTY_SOL)
