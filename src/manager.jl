...
#get_subset(m::Manager{I,T}, Duty::Type{<:AbstractDuty}, stat::Status) where {I <: Id, T} = filter(e -> getduty(getstate(e)) isa Duty && getstate(e).cur_status == stat, m.members)

#get_subset(m::Manager{I,T}, Duty::Type{<:AbstractDuty}) where {I <: Id, T} = filter(e -> getduty(getstate(e)) isa Duty, m.members)

#get_subset(m::Manager{I,T}, stat::Status) where {I <: Id, T} = filter(e -> getstate(e).cur_status == stat, m.members)
