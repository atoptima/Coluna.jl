...
function run_colgen_iteration!(context, phase, stage, env, ip_primal_sol, stab)
    ...
    while misprice
        ...
        valid_db = compute_dual_bound(context, phase, sps_db, generated_columns, cur_mast_dual_sol)
    
        # pseudo dual bound is used for stabilization only.
        pseudo_db = compute_dual_bound(context, phase, sps_pb, generated_columns, cur_mast_dual_sol)
