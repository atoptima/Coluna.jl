...
struct MembersMatrix{I,K,J,L,T} <: AbstractMembersContainer
    col_elements::Dict{I,K}
    row_elements::Dict{J,L}
    cols::Dict{I, MembersVector{J,L,T}}
    rows::Dict{J, MembersVector{I,K,T}}
end

function MembersMatrix{I,K,J,L,T}(col_elems, row_elems) where {I,K,J,L,T}
    cols = Dict{I, MembersVector{J,L,T}}()
    rows = Dict{J, MembersVector{I,K,T}}()
    MembersMatrix(col_elems, row_elems, cols, rows)
end

function _getmembersvector!(dict::Dict{I, MembersVector{J,L,T}}, key::I, elems::Dict{J,L}) where {I,J,L,T}
    if !haskey(dict, key)
...
function Base.setindex!(m::MembersMatrix, val, col_id, row_id)
    cols = _getmembersvector!(m.cols, col_id, m.col_elements)
    cols[row_id] = val
    rows = _getmembersvector!(m.rows, row_id, m.row_elements)
    rows[col_id] = val
    m
end
...
function Base.getindex(m::MembersMatrix, ::Colon, row_id)
    _getmembersvector!(m.rows, row_id, m.row_elements)
end

function Base.getindex(m::MembersMatrix, col_id, ::Colon)
    _getmembersvector!(m.cols, col_id, m.col_elements)
end

function setcolumn!(m::MembersMatrix, col_id, new_col::Dict)
    col = MembersVector(m.col_elements, deepcopy(new_col))
    m.cols[col_id] = col
    for (row_id, val) in col
        row = _getmembersvector!(m.rows, row_id, m.row_elements)
        row[col_id] = val
    end
    m
end
...
function setrow!(m::MembersMatrix, row_id, new_row::Dict)
    ...
    for (col_id, val) in row
        col = _getmembersvector!(m.cols, col_id, m.col_elements)
        col[row_id] = val
    end
    m
end
