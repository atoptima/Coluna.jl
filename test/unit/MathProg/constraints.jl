...
@testset "MathProg - single variable constraint" begin
    form =  ClMP.create_formulation!(Env(Coluna.Params()), ClMP.Original())
    var = ClMP.setvar!(form, "x", ClMP.OriginalVar)
    constr = ClMP.setsinglevarconstr!(
        form, "fake_single_var_constr", ClMP.getid(var), ClMP.OriginalConstr; rhs = -2.0,
        kind = ClMP.Essential, sense = ClMP.Equal, inc_val = -12.0, is_active = true
    )

    cid = ClMP.getid(constr)

    # rhs
    @test ClMP.getcurrhs(form, cid) == -2.0
    ClMP.setcurrhs!(form, cid, -10.0)
    @test ClMP.getcurrhs(form, cid) == -10.0
    @test ClMP.getperenrhs(form, cid) == -2.0

    ClMP.setperenrhs!(form, cid, 33.0)
    @test ClMP.getperenrhs(form, cid) == 33.0
    @test ClMP.getcurrhs(form, cid) == 33.0

    # sense
    @test ClMP.getcursense(form,cid) == ClMP.Equal
    ClMP.setcursense!(form, cid, ClMP.Less)
    @test ClMP.getcursense(form,cid) == ClMP.Less
    @test ClMP.getperensense(form,cid) == ClMP.Equal

    ClMP.setperensense!(form, cid, ClMP.Greater)
    @test ClMP.getperensense(form, cid) == ClMP.Greater
    @test ClMP.getcursense(form, cid) == ClMP.Greater

    # explicit
    @test ClMP.isexplicit(form, cid)
    @test ClMP.isexplicit(form, constr)

    # name
    @test ClMP.getname(form,cid) == "fake_single_var_constr"
end