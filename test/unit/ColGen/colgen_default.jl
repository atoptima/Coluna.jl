...
function test_two_identicals_cols_at_two_iterations_failure()
    ...
    #@test_throws ClA.ColumnAlreadyInsertedColGenWarning 
    ColGen.insert_columns!(reform, ctx, phase, columns)
end
...
function test_colgen_loop()
    ...
    ctx = ClA.ColGenContext(reform, ClA.ColumnGeneration())
    output = ColGen.run_colgen_phase!(ctx, ClA.ColGenPhase3(), env)
    @show output
end
