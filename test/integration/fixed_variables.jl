...
@testset "Integration - fixed variables" begin
    ...
    output = run!(ClA.SolveLpForm(get_dual_solution = true), env, form, OptimizationState(form))

    primal_sol = get_best_lp_primal_sol(output)
    dual_sol = get_best_lp_dual_sol(output)
